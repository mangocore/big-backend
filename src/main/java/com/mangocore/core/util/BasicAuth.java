package com.mangocore.core.util;
...
public class BasicAuth {
    ...
    public static Data createBasicAuthData(String clientId, String clientSecret, String methodName, String pathName) throws NoSuchAlgorithmException, InvalidKeyException {
        String authDate = ZonedDateTime.now(TIME_ZONE).format(TIME_FORMATTER);
        String sign = getHmacSHA1(clientSecret, methodName, pathName, authDate);
        return Data.create(authDate, String.format(AUTH_FORMAT, clientId, sign));
    }

    public static boolean checkBasicAuthData(Data data, String clientSecret, String methodName, String pathName) throws NoSuchAlgorithmException, InvalidKeyException {
        if (!data.getAuth().startsWith(MWS)) {
            return false;
        }
        String sign = getHmacSHA1(clientSecret, methodName, pathName, data.getDate());
        return Objects.equals(sign, data.getAuth().split(CO)[1]);
    }

    private static String getHmacSHA1(String clientSecret, String methodName, String pathName, String authDate) throws NoSuchAlgorithmException, InvalidKeyException {
        String sourceSign = String.format(ENCRYPT_FORMAT, methodName, pathName, authDate);
