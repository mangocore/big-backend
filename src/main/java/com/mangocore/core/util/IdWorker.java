...
public class IdWorker {
     ...
     */
    private static final long originEpoch = 1288834974657L;

    /**
     * 每一部分占用的位数
     */
    private static final long datacenterIdBits = 5L;  //数据标识id所占的位数
    private static final long workerIdBits = 5L;      //机器标识id所占的位数
    ...
    private static final long maxWorkerId = -1L ^ (-1L << workerIdBits);           //支持的最大机器标识id，结果是31
    private static final long sequenceMask = -1L ^ (-1L << sequenceBits);          //支持的最大序列号，结果是4095

    /**
     * 每一部分向左的位移
     */
    private static final long timestampLeftShift = datacenterIdBits + workerIdBits + sequenceBits;     //时间截向左移22位(5+5+12)
    private static final long datacenterIdShift = workerIdBits + sequenceBits;                         //数据标识id向左移17位(12+5)
    private static final long workerIdShift = sequenceBits;                                            //机器标识id向左移12位
    ...
    public synchronized long nextId() {
        ...
        return ((timestamp - originEpoch) << timestampLeftShift)  //时间戳部分
                | (datacenterId << datacenterIdShift)             //数据标识id部分
                | (workerId << workerIdShift)                     //机器标识id部分
                | sequence;                                       //序列号部分
    }
